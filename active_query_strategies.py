    ...
    if substrate_selection == "farthest":
        ...
        while len(next_subs_inds) < n_subs_to_sample:
            sampled_fp = [
                ...
                for x in sampled_smiles
            ]
            rem_fp = [
                mfpgen.GetCountFingerprint(Chem.MolFromSmiles(x)) for x in rem_smiles
            ]
            dists = np.zeros((len(rem_fp), len(sampled_fp)))
            for i, fp in enumerate(rem_fp):
                dists[i] = 1 - np.array(
                    DataStructs.BulkTanimotoSimilarity(fp, sampled_fp)
                )
        ...
        sorted_inds = np.argsort(np.mean(dists, axis=1))[::-1]
        if substrate_selection == "quantile":
            n_to_add = 1
        elif substrate_selection == "skip_one":
            n_to_add = 2
        next_subs_inds = [
            sorted_inds[int(len(sorted_inds) * (x / (n_subs_to_sample + n_to_add)))]
            for x in range(n_to_add, n_subs_to_sample + n_to_add)
        ]
    X_acquired = X[[rem_inds[x] for x in next_subs_inds]]
    y_ranking_acquired = get_y_acquired(
        y_ranking, rem_inds, next_subs_inds, next_cond_inds
...
def iteration_of_two_cond_pairs(
    X,
    y_ranking,
    train_inds,
    rem_inds,
    ...
    next_cond_inds = np.array(
        [[x for x in first_cond_pair], [x for x in other_cond_pair]]
    )
...
def iteration_of_score_overlap(
    ...
    for i, (score_row, std_row) in enumerate(
        zip(np.mean(score_array, axis=2), np.std(score_array, axis=2))
    ):
    ...
    if len(next_subs_inds) == 0:
        # print("DIFFS", diffs)
        subs_inds, cond_inds = np.unravel_index(
            np.argsort(diffs, axis=None), shape=diffs.shape
        )
...
def iteration_of_highest_disagreement(
    ...
    for subs_ind in range(ensemble_of_predicted_proba[0].shape[0]):
        combined_proba_array = np.stack(
            tuple([x[subs_ind, :, :] for x in ensemble_of_predicted_proba]), axis=2
        )
        for (row, col) in combinations(
            range(ensemble_of_predicted_proba[0].shape[1]), 2
        ):
            sub_array = combined_proba_array[row, col]
            pos_inds = np.where(sub_array > 0.5)[0]
            neg_inds = np.where(sub_array <= 0.5)[0]
            disagreement_score_array[subs_ind, row, col] = (
                abs(
                    len(ensemble_of_predicted_proba) - max(len(pos_inds), len(neg_inds))
                )
                + np.mean(sub_array[pos_inds])
                - np.mean(sub_array[neg_inds])
            )
    disagreement_score_array *= -1
    sorted_inds = np.unravel_index(
        np.argsort(disagreement_score_array, axis=None), disagreement_score_array.shape
    )
    next_subs_inds = [x for x in sorted_inds[0][-1 * n_subs_to_sample :]]
    next_cond_inds = [
        [sorted_inds[1][-1 * x], sorted_inds[2][-1 * x]]
        for x in range(n_subs_to_sample)
    ]
    # print(next_subs_inds)
    # print(next_cond_inds)
    # print()
...
def iteration_of_explore_rfr(
    X, y_yield, rem_inds, predicted_yield_array, n_rxns_to_sample
):
    ...
    """
    next_inds = np.argsort(np.std(predicted_yield_array, axis=1))[
        -1 * n_rxns_to_sample :
    ]
