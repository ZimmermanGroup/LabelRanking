...
class IBLR_PL():
    ...
    def _plackett_luce(self,rankings):
        ...
        pp = np.count_nonzero(~np.isnan(rankings), axis=1)  # pp[j] = # individuals in contest j
        ...
        while np.linalg.norm(dgamma) > 1e-09:
            ...
            for i in range(M) :
                ...
                denominator[i] = np.sum(g[np.where(~np.isnan(col))[0], col[~np.isnan(col)]-1])
    ...
    def predict(self, X) :
        ...
        return pl_param_array.shape[1] - np.argsort(np.argsort(pl_param_array, axis=1), axis=1)
