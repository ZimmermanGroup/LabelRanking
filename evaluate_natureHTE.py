...
def parse_args():
    ...
    parser.add_argument(
        "--mcknn", action="store_true", help="Include Multiclass kNN classifier"
    ),
    parser.add_argument(
        "--baseline",
        action="store_true",
...
def load_data(feature_type, output_type, dataset, use_full=True):
    ...
    if use_full is False:
        rows_to_remove_dict = {
            "amide": AMIDE_ROWS_TO_REMOVE,
            "amine": AMINE_ROWS_TO_REMOVE,
            "sulfonamide": SULFON_ROWS_TO_REMOVE,
        }
        substrate_smiles_list = [
            x
            for i, x in enumerate(substrate_smiles_list)
            if i not in rows_to_remove_dict[dataset[0]]
        ]
    ...
    if output_type in ["ranking", "multiclass"]:
        ...
        if use_full is False:
            ...
            y = np.array(
                [val for i, val in enumerate(y) if int(i // 4) not in rows_to_remove]
            )
    return X, y
...
def evaluate_lr_alg(test_yield, test_rank, pred_rank, n_rxns, perf_dict, comp, model):
    ...
    print(
        f"    kendall-tau={kt} // reciprocal_rank={rr} // regret={regret} // mrr={mrr}"
    )
    print()

    update_perf_dict(PERFORMANCE_DICT, kt, rr, mrr, regret, comp, model)
...
def rfr_eval(
    ...
    for i, (train_ind, test_ind) in tqdm(enumerate(ps.split())):
        ...
        if len(np.unique(y_test)) > 1:
            ...
            best_retrieved_yield = np.max(y_test[largest_yield_inds])
            actual_inds_with_that_yield = np.where(y_test == best_retrieved_yield)[0]
            reciprocal_rank = 1 / np.min(y_ranking[actual_inds_with_that_yield])
...
def mc_eval(feature_type, substrate_type, parser, n_rxns=1, random_state=42):
    ...
    def mc_eval(y_rank_test, pred_proba, pred_val, n_rxns, comp, model_name):
        ...
        mrr = np.mean(
            np.reciprocal(
                y_rank_test[np.argsort(pred_proba)[::-1][:n_rxns]], dtype=np.float32
            )
        )
    ...
    X, y = load_data(
        feature_type, "multiclass", substrate_type, use_full=parser.use_full
    )
    _, y_rank = load_data(
        feature_type, "ranking", substrate_type, use_full=parser.use_full
    )
    test_fold = np.arange(X.shape[0])  ## LOOCV
    ps = PredefinedSplit(test_fold)
    for i, (train_ind, test_ind) in enumerate(ps.split()):
...
if __name__ == "__main__":
    ...
    if parser.save:
        ...
        for model in perf_df["model"].unique():
            print(
                model,
                round(perf_df[perf_df["model"] == model]["reciprocal_rank"].mean(), 3),
