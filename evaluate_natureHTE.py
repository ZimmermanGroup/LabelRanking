...
np.random.seed(42)
RAW_DATA = pd.read_excel("datasets/natureHTE/natureHTE.xlsx", sheet_name="Report - Variable Conditions", usecols=["BB SMILES", "Chemistry", "Catalyst", "Base", "Rel. % Conv."])
...
REAGENT_DATA = {}
base_descriptors = pd.read_excel("datasets/natureHTE/reagent_desc.xlsx", sheet_name="Base")
cat_descriptors = pd.read_excel("datasets/natureHTE/reagent_desc.xlsx", sheet_name="Catalyst")
for _, row in base_descriptors.iterrows():
    REAGENT_DATA.update({
        row[0]:row[1:].to_numpy()
    })
    ...
    if feature_type == "fp":
        ...
        if output_type == "yield":
            ...
            for i, row in full_dataset.iterrows():
                fp_portion = fp_array[substrate_smiles_list.index(row["BB SMILES"])].reshape(1,-1)
                cat_descriptors = REAGENT_DATA[row["Catalyst"]].reshape(1,-1)
                base_descriptors = REAGENT_DATA[row["Base"]].reshape(1,-1)
                row_array.append(np.hstack((
                    fp_portion, cat_descriptors, base_descriptors
                )))
        ...
        if output_type == "ranking":
            ...
            X = np.zeros((full_dataset.shape[0], len(substrate_smiles_list)+len(bases)+len(catalysts)))
            for i, row in full_dataset.iterrows():
                X[i, [
        ...
        if output_type == "ranking":
            ...
            substrate_random_fp = np.random.randint(3, size=(n_subs, 1024)) # to simulate count FP
            reagent_random_desc = np.random.rand(4, 12) # there are 12 descriptors
            X = np.zeros((full_dataset.shape[0], 1036))
            for i, row in full_dataset.iterrows():
                X[i] = np.concatenate((
                    substrate_random_fp[substrate_smiles_list.index(row["BB SMILES"])].flatten(),
                    reagent_random_desc[2*catalysts.index(row["Catalyst"])+bases.index(row["Base"])].flatten()
                ))
...
def rfr_eval(
    ...
    for i, (train_ind, test_ind) in tqdm(enumerate(ps.split())):
        ...
        print(f"    kendall-tau={kt} // reciprocal_rank={reciprocal_rank} // mrr={mean_reciprocal_rank}")
        print()
    print()
...
if __name__ == "__main__" :
    ...
    if parser.save:
        joblib.dump(
            PERFORMANCE_DICT,
            f"performance_excels/natureHTE/{subs_type}_{feature}.joblib",
        )